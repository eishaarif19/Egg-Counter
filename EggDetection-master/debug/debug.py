import cv2

cap = cv2.VideoCapture('20180910-144521_rotated_LS.mp4')


import numpy as np

def reScaleFrame(frame, percent=75):
    width = int(frame.shape[1] * percent // 100)
    height = int(frame.shape[0] * percent // 100)
    dim = (width, height)
    return cv2.resize(frame, dim, interpolation=cv2.INTER_AREA)



while(1):import numpy as np
import pprint
import sys
import datetime
import math
import cv2
import matplotlib.pyplot as plt

from dialogs.settings_dialog import Settings
from PyQt5.QtWidgets import QApplication
from getdist import plots, MCSamples


### global variables


width = 0
height = 0
eggCount = 0
exitCounter = 0
OffsetRefLines = 50  # Adjust ths value according to your usage
ReferenceFrame = None
distance_thresh = 100
radius_min = 0
radius_max = 0
area_min = 0
area_max = 0


### importing Settings


app = QApplication(sys.argv)
set = Settings()
#sys.exit(app.exec_())


### checks for detection algorithm to start

def reScaleFrame(frame, percent=75):
    width = int(frame.shape[1] * percent // 100)
    height = int(frame.shape[0] * percent // 100)
    dim = (width, height)
    return cv2.resize(frame, dim, interpolation=cv2.INTER_AREA)


def CheckInTheArea(coordYContour, coordYEntranceLine, coordYExitLine):
    if ((coordYContour <= coordYEntranceLine) and (coordYContour >= coordYExitLine)):
        return 1
    else:
        return 0


def CheckEntranceLineCrossing(coordYContour, coordYEntranceLine):
    absDistance = abs(coordYContour - coordYEntranceLine)

    if ((coordYContour >= coordYEntranceLine) and (absDistance <= 3)):
        return 1
    else:
        return 0


def getDistance(coordYEgg1, coordYEgg2):
    dist = abs(coordYEgg1 - coordYEgg2)

    return dist


### reading video


cap = cv2.VideoCapture("20180910-144521_rotated_LS.mp4")
#cap = cv2.VideoCapture(0)
#cap = cv2.VideoCapture('rtsp://admin:9ejq28Ez@172.16.1.65:554/Streaming/Channels/101?transportmode=unicast&profile=Profile_1')

fgbg = cv2.createBackgroundSubtractorMOG2()  # for mask

while True:

    (grabbed, frame) = cap.read()

    if not grabbed:
        print('Egg count: ' + str(eggCount))
        print('\n End of the video file...')
        break


    ### get Settings radius/area values

    
    radius_min,radius_max = set.getRadius()
    area_min,area_max = set.getArea()

    if radius_min == '':
        radius_min = 0
    if radius_max == '':
        radius_max = 0

    if area_min == '':
        area_min = 0
    if area_max == '':
        area_max = 0

    frame40 = reScaleFrame(frame, percent=40)
    ### count the number of elements along the axis.
    ### Here rows and columns are being treated
    ### as elements

    #cv2.imshow('frame', frame40)
 

    height = np.size(frame40, 0)    ### gives no. of rows along x-axis
    width = np.size(frame40, 1)     ### gives no. of columns along y-axis

    fgmask = fgbg.apply(frame40)

    #cv2.imshow('frame', fgmask)

    hsv = cv2.cvtColor(frame40, cv2.COLOR_BGR2HSV)

    #cv2.imshow('frame', hsv)
    
    
    #cv2.imshow('frame',hsv[:, :, 2])   ### hsv[:, :, 2] is black

    th, bw = cv2.threshold(hsv[:, :, 2], 0, 255, cv2.THRESH_BINARY | cv2.THRESH_OTSU)
    

    #cv2.imshow('frame', bw)
    
    kernel = cv2.getStructuringElement(cv2.MORPH_ELLIPSE, (3, 3))           ### array return type

    #morph = cv2.morphologyEx(bw, cv2.MORPH_CLOSE, kernel)            ### array return type

    morph = cv2.morphologyEx(bw, cv2.MORPH_CLOSE, kernel)
    

    #print(morph)

    dist = cv2.distanceTransform(morph, cv2.DIST_L2, cv2.DIST_MASK_PRECISE)        ### array return type

    #print(dist)

    borderSize = set.getBorderSizeValue()
    
    distborder = cv2.copyMakeBorder(dist, borderSize, borderSize, borderSize,  borderSize,
                                    cv2.BORDER_CONSTANT | cv2.BORDER_ISOLATED, 0)

    #cv2.imshow('frame', distborder)
    
    gap = 10
    kernel2 = cv2.getStructuringElement(cv2.MORPH_ELLIPSE, (2 * (borderSize - gap) + 1, 2 * (borderSize - gap) + 1))
    
    kernel2 = cv2.copyMakeBorder(kernel2, gap, gap, gap, gap,
                                 cv2.BORDER_CONSTANT | cv2.BORDER_ISOLATED, 0)

    distTempl = cv2.distanceTransform(kernel2, cv2.DIST_L2, cv2.DIST_MASK_PRECISE)

    #cv2.imshow('frame', distTempl)

    nxcor = cv2.matchTemplate(distborder, distTempl, cv2.TM_CCOEFF)

    #cv2.imshow('frame', nxcor)

    ###  finds the location with the largest and smallest value

    mn, mx, _, _ = cv2.minMaxLoc(nxcor)
    
    th, peaks = cv2.threshold(nxcor, mx * 0.6, 255, cv2.THRESH_BINARY)


    #cv2.imshow('frame', peaks)
    
    peaks8u = cv2.convertScaleAbs(peaks)

    # fgmask = fgbg.apply(peaks8u)

    cv2.imshow('frame', peaks8u)

    

    contours, hierarchy = cv2.findContours(peaks8u, cv2.RETR_CCOMP, cv2.CHAIN_APPROX_SIMPLE)  ### contours is a vector

    #print(contours)

    ### plot reference lines (entrance and exit lines)

    coordYEntranceLine = (height // 2) + OffsetRefLines
    coordYMiddleLine = (height // 2)
    coordYExitLine = (height // 2) - OffsetRefLines
    cv2.line(frame40, (0, coordYEntranceLine), (width, coordYEntranceLine), (255, 0, 0), 2)
    cv2.line(frame40, (0, coordYMiddleLine), (width, coordYMiddleLine), (0, 255, 0), 6)
    cv2.line(frame40, (0, coordYExitLine), (width, coordYExitLine), (255, 0, 0), 2)


    ### Store the position and a flag of the eggs in an array, egg_list


    flag = False
    egg_list = []
    egg_index = 0

    for i in range(len(contours)):
        contour = contours[i]

        (x, y), radius = cv2.minEnclosingCircle(contour)
        radius = int(radius)

        (x, y, w, h) = cv2.boundingRect(contour)

        egg_index = i

        for k in range(len(egg_list)):
            egg_new_X = x
            egg_new_Y = y

            dist = getDistance(egg_new_Y, egg_list[k][1])
            
            if dist > distance_thresh:  # distance_thresh is 200
                            egg_list.append([egg_new_X, egg_new_Y, flag])
            

        #egg_list.append([x, y, flag])

        print(egg_list)

        if len(contour) >= 5:

            if (radius <= int(radius_max) and radius >= int(radius_min)):

                # print("radius: ", radius)
                # pprint.pprint(hierarchy)

                ellipse = cv2.fitEllipse(contour)

                #print(type(ellipse))
                #print(ellipse)
                
                #(x, y, w, h) = cv2.boundingRect(contour)

                (center, axis, angle) = ellipse
                
                coordXContour, coordYContour = int(center[0]), int(center[1])
                coordXCentroid = (2 * coordXContour + w) // 2
                coordYCentroid = (2 * coordYContour + h) // 2
                ax1, ax2 = int(axis[0]) - 2, int(axis[1]) - 2
                orientation = int(angle)
                area = cv2.contourArea(contour)

                if area >= int(area_min) and area <= int(area_max):
                    #print('egg list: ' + str(egg_list) + ' index: ' + str(egg_index))

                    if CheckInTheArea(coordYContour, coordYEntranceLine, coordYExitLine):
                        cv2.ellipse(frame40, (coordXContour, coordYContour), (ax1, ax2), orientation, 0, 360, (255, 0, 0), 2)  # blue
                        
                        cv2.circle(frame40, (coordXContour, coordYContour), 1, (0, 255, 0), 15)  # green
                        cv2.putText(frame40, str(int(area)), (coordXContour, coordYContour), cv2.FONT_HERSHEY_SIMPLEX,
                                    0.5, 0, 1, cv2.LINE_AA)

                    #for k in range(len(egg_list)):
                        #egg_new_X = x
                       # egg_new_Y = y
                        
                        ### Find all the eggs in the second frame and
                        ### compare it with the egg_list. if the distance
                        ### is below some predefined value, consider them
                        ### as the same egg. Else add the new egg to the
                        ### egg_list.
                        
                       # dist = getDistance(egg_new_Y, egg_list[k][1])

                       # if dist > distance_thresh:  # distance_thresh is 200
                         #   egg_list.append([egg_new_X, egg_new_Y, flag])

                    if CheckEntranceLineCrossing(egg_list[egg_index][1], coordYMiddleLine) and not egg_list[egg_index][2]:
                        eggCount += 1
                        egg_list[egg_index][2] = True

                cv2.putText(frame40, "Entrance Eggs: {}".format(str(eggCount)), (10, 100), cv2.FONT_HERSHEY_SIMPLEX, 0.5,
                            (250, 0, 1), 2)

    cv2.imshow("Original Frame", frame40)

    key = cv2.waitKey(1)

    if key == 27:
        break


### cleanup the camera and close any open windows

    
cap.release()
cv2.destroyAllWindows()
